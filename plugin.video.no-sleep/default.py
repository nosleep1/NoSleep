...
#############Defined Strings#############
addon_id     = 'plugin.video.no-sleep'
...
Guide = xbmc.translatePath(os.path.join('special://home/addons/plugin.video.no-sleep/resources/catchup', 'guide.xml'))
GuideLoc = xbmc.translatePath(os.path.join('special://home/addons/plugin.video.no-sleep/resources/catchup', 'g'))
...
def start():
	if username=="":
		...
		if auth == "":
			...
			line2 = "Welcome to No-Sleep" 
			line3 = ('[COLOR ghostwhite]%s[/COLOR]'%user)
			xbmcgui.Dialog().ok('No-Sleep', line1, line2, line3)
			tvguidesetup()
			addonsettings('ADS2','')
			xbmc.executebuiltin('Container.Refresh')
			home()
...
##########################################
def catchup():
    loginurl   = "http://gehosting.uk:25461/get.php?username=" + username + "&password=" + password + "&type=m3u_plus&output=ts"
    try:
...
def listcatchup():
	...
	for a in all:
		if '"tv_archive":1' in a:
			...
			tools.addDir(name.replace('ENT:','[COLOR ghostwhite]ENT:[/COLOR]').replace('DOC:','[COLOR ghostwhite]DOC:[/COLOR]').replace('MOV:','[COLOR ghostwhite]MOV:[/COLOR]').replace('SSS:','[COLOR ghostwhite]SSS[/COLOR]').replace('BTS:','[COLOR ghostwhite]BTS:[/COLOR]').replace('TEST','[COLOR ghostwhite]TEST[/COLOR]').replace('Install','[COLOR ghostwhite]Install[/COLOR]').replace('24/7','[COLOR ghostwhite]24/7[/COLOR]').replace('INT:','[COLOR ghostwhite]INT:[/COLOR]').replace('DE:','[COLOR ghostwhite]DE:[/COLOR]').replace('FR:','[COLOR ghostwhite]FR:[/COLOR]').replace('PL:','[COLOR ghostwhite]PL:[/COLOR]').replace('AR:','[COLOR ghostwhite]AR:[/COLOR]').replace('LIVE:','[COLOR ghostwhite]LIVE:[/COLOR]').replace('ES:','[COLOR ghostwhite]ES:[/COLOR]').replace('IN:','[COLOR ghostwhite]IN:[/COLOR]').replace('PK:','[COLOR ghostwhite]PK:[/COLOR]'),'url',13,thumb,fanart,id)
			

def tvarchive(name,description):
    name = str(name.replace('[COLOR ghostwhite]ENT:[/COLOR]','ENT:').replace('[COLOR ghostwhite]DOC:[/COLOR]','DOC:').replace('[COLOR ghostwhite]MOV:[/COLOR]','MOV').replace('[COLOR ghostwhite]SSSS[/COLOR]','SSS:').replace('[COLOR ghostwhite]BTS:[/COLOR]','BTS:').replace('[COLOR ghostwhite]INT:[/COLOR]','INT:').replace('[COLOR ghostwhite]DE:[/COLOR]','DE:').replace('[COLOR ghostwhite]FR:[/COLOR]','FR:').replace('[COLOR ghostwhite]PL:[/COLOR]','PL:').replace('[COLOR ghostwhite]AR:[/COLOR]','AR:').replace('[COLOR ghostwhite]LIVE:[/COLOR]','LIVE:').replace('[COLOR ghostwhite]ES:[/COLOR]','ES:').replace('[COLOR ghostwhite]IN:[/COLOR]','IN:').replace('[COLOR ghostwhite]PK:[/COLOR]','PK'))
    filename = open(Guide)
        ...
        if name in programme.attrib.get('channel'):
            ...
            if head > date:
                if head < now:
                    ...
                    head2 = '[COLOR ghostwhite]%s - [/COLOR]'%head2 
...
def _pbhook(numblocks, blocksize, filesize, dp, start_time):
        ...
        if dp.iscanceled():
            ...
            dialog.ok("no-sleep", 'The download was cancelled.')
...
def searchdialog():
	search = control.inputDialog(heading='Search No-Sleep:')
	if search=="":
		return
	else:
		return search
...
def addonsettings(url,description):
	if   url =="CC":
		...
		if dialog==0:
			...
			xbmcgui.Dialog().ok('No-Sleep', 'Set Advanced Settings')
		elif dialog==1:
			...
			xbmcgui.Dialog().ok('No-Sleep', 'Advanced Settings Removed')
	elif url =="ADS2":
		...
		if dialog==0:
			ivueint()
			xbmcgui.Dialog().ok('No-Sleep', 'iVue Integration Complete')
		elif dialog==1:
			pvrsetup()
			xbmcgui.Dialog().ok('No-Sleep', 'PVR Integration Complete')
		elif dialog==2:
			pvrsetup()
			ivueint()
			xbmcgui.Dialog().ok('No-Sleep', 'PVR & iVue Integration Complete')
	elif url =="ST":
		xbmc.executebuiltin('Runscript("special://home/addons/plugin.video.no-sleep/resources/modules/speedtest.py")')
	elif url =="META":
...
def asettings():
	choice = xbmcgui.Dialog().yesno('No-Sleep', 'Please Select The RAM Size of Your Device', yeslabel='Less than 1GB RAM', nolabel='More than 1GB RAM')
	if choice:
		lessthan()
	else:
		morethan()
...
def accountinfo():
	...
	try:
		...
		for day,month,year in expreg:
			...
			tools.addDir('[COLOR ghostwhite]Username :[/COLOR] '+username,'','',icon,fanart,'')
			tools.addDir('[COLOR ghostwhite]Password :[/COLOR] '+password,'','',icon,fanart,'')
			tools.addDir('[COLOR ghostwhite]Expiry Date:[/COLOR] '+expiry,'','',icon,fanart,'')
			tools.addDir('[COLOR ghostwhite]Account Status :[/COLOR] %s'%status,'','',icon,fanart,'')
			tools.addDir('[COLOR ghostwhite]Current Connections:[/COLOR] '+ active,'','',icon,fanart,'')
			tools.addDir('[COLOR ghostwhite]Allowed Connections:[/COLOR] '+connects,'','',icon,fanart,'')
			tools.addDir('[COLOR ghostwhite]Local IP Address:[/COLOR] '+ip,'','',icon,fanart,'')
			tools.addDir('[COLOR ghostwhite]External IP Address:[/COLOR] '+extip,'','',icon,fanart,'')
	except:pass
		
	
def correctPVR():

	addon = xbmcaddon.Addon('plugin.video.no-sleep')
	...
	loginurl   = "http://gehosting.uk:25461/get.php?username=" + username_text + "&password=" + password_text + "&type=m3u_plus&output=ts"
	EPGurl     = "http://gehosting.uk:25461/xmltv.php?username=" + username_text + "&password=" + password_text + "&type=m3u_plus&output=ts"
...
def tvguidesetup():
		dialog = xbmcgui.Dialog().yesno('No-Sleep','Would You like us to Setup the PVR Guide for You?')
		if dialog:
